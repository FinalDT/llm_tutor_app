...
# 유사문제 힌트 구현하는 과정에서 정답 판별 기능에도 문제가 생김.
# Function App을 초기화합니다.
app = func.FunctionApp(http_auth_level=func.AuthLevel.FUNCTION)

# 헬퍼 함수: 세션 결과(여러 행)를 LLM이 읽기 쉬운 텍스트로 변환
def format_session_results_for_llm(rows):
    """DB 조회 결과를 LLM이 읽기 쉬운 텍스트로 변환합니다."""
    summary = []
...
def tutor_api(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # --- [기능 2] 개별 문항 피드백 (힌트/코칭/유사문항) ---
            ...
            cursor.execute("SELECT assessmentItemID FROM gold.vw_personal_item_enriched WHERE learnerID = ? AND session_id = ? AND seq_in_session = ?", learner_id, session_id, question_number)
            ...
            # 💡 [수정] 더 구체적인 의도를 먼저 확인하도록 순서 변경 및 키워드 추가
            intent = "feedback_request" # 기본값
            if "비슷한 문제" in student_message or "연습 문제" in student_message or "유사 문항" in student_message or "유사문항" in student_message:
                intent = "similar_item_request"
            elif "힌트" in student_message or "모르겠어" in student_message:
                intent = "hint_request"
